...
LLM 生成的 proposal 不应直接触达有副作用的工具；它必须通过适用的结构验证、知识事实与语义前置条件检查、独立策略授权及必要的人工审批，成功后才交给受控执行器，具体门禁顺序按风险、隐私与架构决定。

## Explanation

结构验证检查类型、必填字段和值域；知识图谱与 OWL 检查实体身份、类型和语义前置条件；策略引擎根据主体、动作、资源和上下文决定允许或拒绝；高成本、高风险或不可逆操作还需人工批准。敏感事实读取可以要求先授权，部分低成本结构检查也可以前置。执行器只接收获批提案，并把真实参数与结果写入 trace。

## Caveats

Agent 只生成 proposal，不能为自己的提案提供确定性安全保证。不能把示例门禁顺序固化为唯一流程；只读、低风险工具可以采用较轻门禁，但风险等级和审批规则必须由实际系统定义。
