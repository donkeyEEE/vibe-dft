"""Extract VASP band data as a band-width reference; does not select Wannier windows."""
...
def range_of_band_vaspkit(nk,nb,filename,spin=0):
    ...
    while i < nbe:
        ...
        if spin == 1:
            energy = float(bandfile[i].split()[2])
        band.append(energy)
        i+=1
    Emin = min(band)
    Emax = max(band)
...
def range_of_all_bands(ifspin=False):
    ...
    if os.path.isfile(vestbandname):
        ...
        if ifspin:
            for spin in [0,1]:
                print(f'get bandrange of spin {spin}')
                write2txt(f'bandrange_spin{spin}.dat','      No.   Min     Max')
                nb_list=[]
                EMIN =[]
                EMAX=[]
                for i in range(0,int(nb)):
                    Emin,Emax=range_of_band_vaspkit(nk,i+1,filename,spin)
                    nb_list.append(i+1)
                    EMIN.append(Emin+Efermi)
                    EMAX.append(Emax+Efermi)
                    write2txt(f'bandrange_spin{spin}.dat','Nband: %.f %.3f %.3f'%(i+1,Emin+Efermi,Emax+Efermi))
        else:
            write2txt('bandrange.dat','      No.   Min     Max')
            nb_list=[]
            EMIN =[]
...
while True:
    ...
    if  '1' == project :
        ...
        print('Do you want to enable spin? (y/n)')
        spin_choice = str(input()).lower()
        ifspin = spin_choice == 'y'
        range_of_all_bands(ifspin)
    elif project == '6':
        wcc_output()
    elif project == '7':
        QEbandstructure()
