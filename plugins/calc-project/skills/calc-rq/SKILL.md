...
负责 RQ 生命周期及其已接受决策。已配置 Tracker 只是存储约定；`RQ.md` 是 RQ 的唯一权威。
...
2. 创建时读取已解析主线下的 sibling RQ 目录以避免 ID 冲突。既有 RQ 工作时读取所选 `RQ.md`；
   ...
   不保留单独的问题记录、Tracker 文档、registry、cache、claim 或会话状态。
...
6. 每次写入后重读 `RQ.md`。当已接受决策足以覆盖预期回答范围，且用户未完成请求包含 Spec 设计时，
   直接进入 `$calc-to-spec`，携带已解析 RQ、路径、已接受决策和完整剩余意图，使其能在任何默认
   执行交接前设计所有必要 Spec。缺少批准或配置的 Tracker 不可用时停止。稳定配置或 Spec 设计问题
