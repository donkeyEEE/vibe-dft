---
name: show-cot
description: 展示计算项目的完整计算归属树（COT），并可按用户确认生成 HTML 进展报告。
---
...
在对话中展示完整 COT 后，询问用户是否需要 HTML 形式的详细进展报告。无论本 skill
由用户直接调用、由进度查询进入，还是由 `$calc-execute` 在交还控制前调用，都执行该询问；
只有本次调用已得到用户确认时才生成报告。

用户确认后调用 `$show-me`，根据刚读取的同一组权威记录生成单文件、离线可打开的 HTML。
