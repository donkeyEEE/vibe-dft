...
读取项目的计算配置和 `.calc-project/tracker.json`，根据索引展示已记录进度，日常查询
无需逐篇读取 RQ 或 Spec，也不要求全量扫描以验证索引新鲜度。索引缺失、损坏、条目异常、
已知未同步或用户要求核对时，按项目 `AGENTS.md` 回查权威记录并直接修复；缺失或损坏时
从配置范围内全部 `RQ.md`、已发布 Spec 及其中的 Task、Run 重建。读取
[进度跟踪表契约](../../resources/progress-tracker.md)处理字段与重建；项目约定缺失时，
规则补齐交由 `$calc-setup` 按其授权规则处理，本次查询可继续，不自行修改项目 `AGENTS.md`。
无法完成修复时明确展示覆盖范围和未同步项，不将局部索引称为完整项目。
将完整索引渲染为稳定树：
...
决定生成后调用 `$show-me`，根据本次读取的同一份索引生成单文件、离线可打开的 HTML，
注明索引更新时间；仅为索引未包含的依赖或详细记录按需读取相关 Spec。
