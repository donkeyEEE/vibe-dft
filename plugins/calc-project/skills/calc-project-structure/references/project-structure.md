...
Before writing, clarify only project-specific terms that affect the proposed
structure. Insert confirmed project terms by topic into the single `Language`
section; they may directly replace standard definitions without an override
marker. If `CONTEXT.md` already exists, preserve it unless the user confirms the
