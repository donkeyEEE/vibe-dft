...
The source-preparation utility accepts only explicit input kinds: local files are copied
to `sources/original/`, pasted text and notes are archived under that directory, and URLs
are recorded in `sources/urls.txt` without downloading them. Converted Markdown belongs in
`sources/normalized/`. Never infer that pasted text is a file path.

For a selected PDF panel, use `figure_assets.render_crop` with a zero-based page index and
a rectangle fully contained in the page. It renders only that rectangle at the requested
DPI into the canonical `project_root/images/` directory and always records `crop_qa: pending`.
The `images` directory must not be a symlink, and resolved output paths must remain inside the
project root. The current main agent must visually inspect the result and call `confirm_crop`
with every required preserved element; missing one element is a failed verification. Do not
use pixel heuristics to mark a crop as passing.

The JSON asset manifest stores the canonical source PDF, zero-based page index, four-float
crop rectangle, source label, slide ID, preserved and verified elements, crop-QA state,
project-relative image path, and pixel dimensions. Use `load_asset_manifest` when reading it
back so paths and tuple-valued metadata retain their intended types.

At runtime, `AssetRecord.path` is the canonical absolute image path while `output_path` is
the project-relative manifest path. `confirm_crop` reopens the image before approval and
rejects missing, non-image, escaped, dimension-mismatched, or SHA-256-mismatched files;
metadata alone can never make a crop pass. The manifest's `output_sha256` is required and
