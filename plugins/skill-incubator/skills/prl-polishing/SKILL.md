...
the exact skill-owned file. The local
...
Read these skill-owned references on each applicable invocation. If an optional
reference is unavailable or malformed, warn and continue the polishing task
without substituting another resource source. A missing declared reference is
a plugin packaging defect.
