...
- Start by assigning evidence roles and dependencies; classify the visual archetype only after the scientific chain is complete.
- Prefer one **hero signature panel** plus subordinate prerequisite, validation, control, sensitivity, criterion, or consequence panels over filling the canvas with equal-sized subplots.
...
- Treat journal aesthetics as a secondary layer. For PRL work, evidence compression and final-size legibility take priority over imitation of another journal's visual archetype.
