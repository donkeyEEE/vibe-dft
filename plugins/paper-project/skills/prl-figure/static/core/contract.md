...
For plotting tasks, first honor an explicit Python/R choice in the current request or a clearly language-specific input file/workflow. Save that backend as the user's default with `scripts/prl_figure_backend.py set python` or `scripts/prl_figure_backend.py set r`.

If the current request does not specify a backend, check the saved preference with `scripts/prl_figure_backend.py get`. If it returns `python` or `r`, use that backend without asking again.
...
2. **Evidence chain**: assign each planned panel a primary role (`prerequisite`, `signature`, `validation`, `control`, `sensitivity`, `criterion`, or `consequence`) and drop panels that do not carry a unique piece of evidence.
...
Before layout, apply `static/core/prl-evidence-logic.md`. In particular, do not proceed with mechanism wording without a discriminating control, a quantitative headline without its operational criterion, or a theoretical prediction without its condition and observable consequence.
