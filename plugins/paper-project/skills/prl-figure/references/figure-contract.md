...
Final size:
Panel map:
  a: <evidence role + question answered>
  b: <evidence role + question answered>
  c: <evidence role + question answered>
Evidence hierarchy:
  hero evidence:
  validation evidence:
  controls/robustness:
...
Use this dependency order unless the manuscript story clearly requires another:
...
For a compact PRL figure, translate the generic order into explicit evidence roles from `static/core/prl-evidence-logic.md`. The minimum defensible sequence depends on the claim type; do not add panels merely to complete a template.
