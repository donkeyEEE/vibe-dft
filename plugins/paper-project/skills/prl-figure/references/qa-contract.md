...
| Evidence roles | Every panel has one primary role: prerequisite, signature, validation, control, sensitivity, criterion, or consequence |
| Claim boundary | Prediction, observation, interpretation, and mechanism language match the evidence actually shown |
| Mechanism control | A mechanism claim includes a comparator that discriminates against a plausible alternative, or the residual confounder is stated |
| Quantitative criterion | Thresholds, fits, transitions, extracted quantities, units, and uncertainty are operationally defined |
| Model sensitivity | Assumptions, parameter range, and construction dependence are shown when they change the interpretation |
| Main/supplement allocation | No indispensable logical link, control, or robustness result is hidden only in Supplemental Material |
...
```bash
# Python source
python skills/prl-figure/scripts/validate_figure.py path/to/figure.py

# R, R Markdown, or Quarto source
python skills/prl-figure/scripts/validate_figure.py path/to/figure.R

# Machine-readable report or stricter warning gate
python skills/prl-figure/scripts/validate_figure.py path/to/figure.py --json
python skills/prl-figure/scripts/validate_figure.py path/to/figure.py --strict
```
