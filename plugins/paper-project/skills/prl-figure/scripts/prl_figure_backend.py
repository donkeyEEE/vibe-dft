#!/usr/bin/env python3
"""Read or write the user's default prl-figure plotting backend."""
...
def config_path() -> Path:
    override = os.environ.get("PRL_FIGURE_CONFIG")
    if override:
        return Path(override).expanduser()
    return Path("~/.config/prl-skills/prl-figure.json").expanduser()
...
def read_config(path: Path) -> dict:
    ...
    try:
        ...
        raise SystemExit(f"Invalid prl-figure config JSON at {path}: {exc}") from exc
    if not isinstance(data, dict):
        raise SystemExit(f"Invalid prl-figure config at {path}: expected object")
    return data
