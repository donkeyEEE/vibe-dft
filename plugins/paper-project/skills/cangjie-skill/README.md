...
它不生成可安装 Skill，也不是摘要工具。它先理解来源、进行多视角候选提取和验证，再由 AI 提议、用户确认卡片的类型、tags 和写入动作。通用卡片写入 `/home/donk/plugins/research-knowledge/cards/atoms/`；物理卡片按概念、现象、理论与模型分类，并由独立物理索引导航。
