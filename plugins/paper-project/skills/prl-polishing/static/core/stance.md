...
- On first contact with the draft, build a Terminology Ledger and keep terms, abbreviations, units, and notation consistent across every section. Do not introduce synonyms to vary the prose. After following the shared-library protocol, load ATOM CARD `write-terminology-ledger`.
...
After following the shared-library protocol, load ATOM CARD `write-reader-question-sequence` for the five-step reader question sequence. Polishing should help the paper answer those questions in order.
